...
```java
IP: 10.85.172.36     NetMask: 255.255.254.0
Broadcast: 10.85.172.36 & (255.255.254.0) | !(255.255.254.0)
             = 10.85.173.255 
```
...
最后的Option报文的组织形式为：
>OptionNum(4) + OptionLength(4) + OptionContains(length)
...
### 其他一些注意事项和技巧：
- NAK报文一般只在下列情况下发送
>1.续租请求非法（租约过期、无法找到旧租约、请求的IP分配给了其他设备）
>2.Giaddr为空
