...
>可能到这儿你有点迷茫，这怎么又扯到多项式去了。这里具体为啥域内的元素可以转化为多项式，估计你得问问数学家们这么做有什么好处了。现阶段\\(GF(2^8)\\)域中元素可以这样转化为多项式，比如\\(GF(2^8)\\)中元素\\(113\\)使用2进制表示为\\(01110001_{(2)}\\)，数学家们发明了一种神奇的记法来记这个数字：\\(x^6+x^5+0\times x^4+0\times x^3+x^2+x^1+x^0=x^6+x^5+x^2+x+1\\)。
